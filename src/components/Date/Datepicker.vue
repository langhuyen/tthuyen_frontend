  ...
  <div class="calendar-container">
    <div class="calendar">
      <div class="select-year">
        <div class="calendar-date" :style="{ 'background-color': color }">
          <div>
            <div
              ...
              v-if="showLeftArrow"
            >
              <span width="24" height="24">&lt;&lt;</span>
            </div>
          </div>

          <div class="calendar-current-date flex-center" @click="setSelecting('year')">
      ...
      <!-- Calendar -->
      <div class="calendar-body" @click="selecting='date'">
        <table>
          <thead>
            <tr>
              <td>Thứ 2</td>
      ...
      <div class="calendar-footer" v-if="!footer">
        ...
        <button :style="{ 'color': color }" @click.stop.prevent="onNow">Hôm nay</button>
      </div>
    </div>
  </div>
  <!-- </div> -->
  ...
  computed: {
    ...
    calendar() {
      ...
      if (startOfMonthDay != 0) {
        for (let i = 0, len = startOfMonthDay; i < len - 1; i++) {
          days.push("");
        }
      } else {
        for (let i = 0; i < 6; i++) {
  ...
  methods: {
    ...
    onComplete(e) {
      ...
      const date = this.format
        ...
        : this.calculatedDate;
      var dateTime = date + " " + maskRef.unmaskedValue;
...
.calendar-header h2.calendar-faint,
.calendar-header h3.calendar-faint {
  cursor: pointer;
}
